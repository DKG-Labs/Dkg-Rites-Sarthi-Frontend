...
export const formatCorrectionText = (text) => {
  if (!text) return '';
  let s = String(text);
  // Replace tildes commonly used in railway addresses/names
  s = s.replace(/~/g, ', ');
    ...
    try {
      ...
      try {
        ...
        if (storeRes?.compressedBase64) {
          ...
          console.log(`✅ Compressed pre-sign PDF (Railpad): ${storeRes.originalSize} -> ${storeRes.compressedSize} bytes`);
        }
      } catch (storeErr) {

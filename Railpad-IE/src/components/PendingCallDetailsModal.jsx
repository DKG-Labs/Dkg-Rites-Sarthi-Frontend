...
const PendingCallDetailsModal = ({
  ...
  const downloadPoDoc = async () => {
    ...
    // Extract bare PO number if formatted like "NWR / 63265009202191 / 001" or "63265009202191/001"
    ...
    try {
      const baseUrl = getBaseUrl();
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${baseUrl}/vendor/po-pdf-path?rawPoNo=${encodeURIComponent(barePoNo)}`, {
        headers: {
          'Content-Type': 'application/json',
          ...(token && { 'Authorization': `Bearer ${token}` })
        }
      ...
      if (pdfPath.startsWith('http') || pdfPath.includes('ireps.gov.in')) {
        const proxyUrl = `${baseUrl}/vendor/proxy-pdf?url=${encodeURIComponent(pdfPath)}`;
  ...
  // Format PO Number (e.g. NFR / 61255256102272 / 001)
  const formatFullPo = () => {
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '24px' }}>
              <div>
                ...
                <div style={{ display: 'inline-block', background: '#e0f2fe', color: '#0284c7', padding: '5px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: '700' }}>
                  {call.product_type || call.productType || call.railPadType || 'Process'}
                </div>
              </div>
              <div>

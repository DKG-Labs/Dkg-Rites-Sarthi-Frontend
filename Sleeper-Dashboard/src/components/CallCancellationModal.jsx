...
const CallCancellationModal = ({
  ...
  const fetchPoDetailsForCall = async (requestId, poNo, itemSrNo) => {
    ...
    if (requestId) {
      try {
        ...
        console.warn('Could not fetch sleeper summary:', e);
      }
    }
  ...
  useEffect(() => {
    ...
    const loadCallAndPoDetails = async () => {
      ...
      const rawSr = call.po_sr || call.poSerialNo || call.itemSrNo || call.poSrNo || call.srNo || '';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const callNumber = call.call_no || call.callNumber || call.requestId || call.icNumber || call.callNo;
      const safeCallNo = String(callNumber || '').replace(/[/\\?%*:|"<>]/g, '_');
      ...
      const workflowActionData = {
        ...
        requestId: callNumber,
        action: 'CANCEL',
        remarks: finalRemarks,
        actionBy: Number(userId),
        pincode: call.pincode || call.pinCode || call.pincode_no || '',
        vendorCode: call.vendor_code || call.vendorCode || call.created_by || '',
      ...
      const res = await apiService.performTransitionAction(workflowActionData);

      const msg = cancellationBasis === 'CHARGEABLE'
        ? `✓ Call ${callNumber} cancelled successfully. Financial liability of ₹${finalCancellationCharges.toLocaleString('en-IN')} logged against vendor.`
        : `✓ Call ${callNumber} cancelled on non-chargeable basis.`;
  ...
  const currentCallNumber = call.call_no || call.callNumber || call.requestId || call.icNumber || call.callNo;

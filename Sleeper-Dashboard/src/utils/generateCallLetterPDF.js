...
/**
 * Generates the official Online Inspection Call Letter PDF matching Vendor requested calls format.
 * 
...
export const generateCallLetterPDF = (call, shouldDownload = true) => {
    ...
    const callTypeUpper = (merged.callType || merged.typeOfCall || 'FINAL').toUpperCase();
    ...
    // Item details
    let itemDescStr = merged.itemDesc || merged.itemDescription || 'MANUFACTURE AND SUPPLY OF PRESTRESSED MONO-BLOCK CONCRETE LINE SLEEPERS (RT-8746) (PRETENSIONED TYPE) FOR BROAD GAUGE(1673 MM)';
    ...
    const stageDisplay = 'Final Inspection';
    ...
    const prodSelectedVendor = merged.productSelectedByVendor || merged.product || merged.productType || 'Sleeper';
    ...
    if (Array.isArray(merged.lots) && merged.lots.length > 0) {
        ...
        merged.lots.forEach((l, idx) => {
            ...
            lines.push(`${lotNo}${lotSize ? ` (Qty: ${lotSize})` : ''}`);
        });
